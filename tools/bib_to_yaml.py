...
Before converting, the script also scrubs publications.bib in place,
removing fields that can leak private details from the exporting computer
(local PDF paths, usernames, personal email addresses in cloud-drive folder
names, private notes). See PRIVATE_FIELDS below.
"""
...
# Fields removed from publications.bib before it is committed or published.
# "file" holds local PDF paths such as /Users/<name>/Zotero/storage/...
PRIVATE_FIELDS = {"file", "annote", "annotation", "mendeley-tags",
                  "mendeley-groups", "local-url", "bdsk-file-1", "bdsk-url-1"}

# Any other field whose value looks like a path on someone's computer
# is removed too.
LOCAL_PATH = re.compile(r"(/Users/|/home/|[A-Za-z]:\\\\|CloudStorage|GoogleDrive-)")
...
def scrub_bib(text):
    """Return text with private fields (see PRIVATE_FIELDS) removed."""
    out, pos = [], 0
    field = re.compile(r"(?m)^[ \t]*([\w-]+)[ \t]*=[ \t]*")
    for m in field.finditer(text):
        ...
        if i < len(text) and text[i] == "{":
            ...
            j = text.find('"', i + 1) + 1 or len(text)
        else:
            j = i
            while j < len(text) and text[j] not in ",\n}":
                j += 1
        value = text[i:j]
        if m.group(1).lower() in PRIVATE_FIELDS or LOCAL_PATH.search(value):
            # Drop the field, its trailing comma and the rest of its line.
            k = j
            ...
            pos = k
    out.append(text[pos:])
    # Tidy any comma left dangling before an entry's closing brace.
    return re.sub(r",(\s*\n\})", r"\1", "".join(out))
...
def main():
    ...
    scrubbed = scrub_bib(text)
    if scrubbed != text:
        with open(path, "w", encoding="utf-8") as fh:
            fh.write(scrubbed)
        print(f"Removed private fields (local file paths etc.) from {path}")
        text = scrubbed
